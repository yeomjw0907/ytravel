...
import { formatDateShort, todayISO, tomorrowISO, nightsBetween } from "@/lib/search/format";
...
/**
 * 체크인 / 체크아웃 범위 선택. 트리거에 한국어 날짜·숙박 일수 표시, 팝오버에서 기간 선택.
 * 폼 제출 시 name으로 checkIn/checkOut 전달. 외부 라이브러리 없음.
 */
export function DateRangeField({
  ...
  const [checkOut, setCheckOut] = useState(defaultValueCheckOut || tomorrowISO());
  ...
  const displayText =
    checkIn && checkOut
      ...
      : "날짜 선택";
  const nightsText = nights > 0 ? `${nights}박` : null;
  ...
  return (
    <div className="relative flex flex-col gap-wt-1" ref={popRef}>
      ...
      >
        <span className={checkIn && checkOut ? "" : "text-wt-text-secondary"}>{displayText}</span>
      ...
      {open && (
        <div
          className="absolute top-full left-0 z-20 mt-wt-1 w-full min-w-[280px] max-w-[calc(100vw-2.5rem)] rounded-wt-lg border border-wt-border bg-wt-panel shadow-wt-soft md:left-0 md:min-w-[320px]"
          role="dialog"
          aria-label="체크인·체크아웃 날짜 선택"
        >
          <div className="border-b border-wt-border px-wt-4 py-wt-3">
            <p className="text-wt-body-sm font-medium text-wt-text-primary">숙박 기간 선택</p>
          ...
          <div className="flex flex-col gap-wt-4 p-wt-4">
            ...
            {nights > 0 && (
              <p className="text-wt-caption text-wt-text-secondary">숙박 일수: {nights}박</p>
            )}
